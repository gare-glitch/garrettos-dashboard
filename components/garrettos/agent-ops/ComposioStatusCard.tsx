  ...
  const { data, source, warning } = useGarrettOSData<IntegrationsPayload>(
    ...
    () => ({
      ...
      composio: {
        ...
        cliMode: true,
        mcpMode: false,
        connectedAccounts: ['github', 'gmail', 'googledrive', 'linkedin', 'instagram'],
        connections: [
          { toolkit: 'github', status: 'ACTIVE', wordId: 'github_azine-bahay' },
          { toolkit: 'gmail', status: 'ACTIVE', wordId: 'gmail_unbank-sandix' },
          { toolkit: 'googledrive', status: 'ACTIVE', wordId: 'googledrive_venin-horst' },
          { toolkit: 'linkedin', status: 'ACTIVE', wordId: 'linkedin_tenure-areel' },
          { toolkit: 'instagram', status: 'ACTIVE', wordId: 'instagram_glub-volta' },
          { toolkit: 'outlook', status: 'EXPIRED' },
          { toolkit: 'canvas', status: 'EXPIRED' },
        ],
        toolkits: ['github', 'gmail', 'googledrive', 'linkedin', 'instagram'],
        status: 'connected',
        tone: 'good',
        note: '5 ACTIVE connection(s): github, gmail, googledrive, linkedin, instagram',
      },
    }),
  );
